...
class TestParseFromUrlHandler:
    ...
    async def test_invalid_user_id(self):
        ...
        result = await parse_from_url_handler(self.url, "invalid_user_id", self.logger, self.db_api)
        assert result.status_code == 400
    ...
    async def test_unsupported_url(self, mock_parser):
        ...
        result = await parse_from_url_handler(self.url, self.user_id, self.logger, self.db_api)
        assert result.status_code == 400
        assert result.detail == "Unsupported URL"
    ...
    async def test_failed_to_fetch_receipt(self, mock_parser):
        ...
        result = await parse_from_url_handler(self.url, UUID(self.user_id), self.logger, self.db_api)
        assert result.status_code == 400
    ...
    async def test_receipt_successfully_processed(self, mock_get_html, mock_parser):
        ...
        mock_parser_instance.parse_html.return_value.build_receipt.return_value = mock_receipt
        result = await parse_from_url_handler(self.url, UUID(self.user_id), self.logger, self.db_api)
